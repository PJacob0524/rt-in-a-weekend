...
impl Scene {
    pub fn load(scene_name: &str) -> Option<Self> {
        match scene_name {
            "weekend" => {
                ...
                for x in -11..11 {
                    for z in -11..11 {
                        ...
                        let center = Point3(
                            x as f64 + 0.9 * rng.gen_range(0.0..1.0),
                            0.2,
                            z as f64 + 0.9 * rng.gen_range(0.0..1.0),
                        );

                        if (center - Point3(4.0, 0.2, 0.0)).length() > 0.9 {
                            if randmat < 0.8 {
                                let mat = Matte {
                                    color: Color::random(0.0, 1.0) * Color::random(0.0, 1.0),
                                };
                                let obj = Sphere::new(center, 0.2, Box::new(mat));
                                world.push(Box::new(obj));
                            } else if randmat < 0.95 {
                                let mat = Metal {
                                    color: Color::random(0.5, 1.0),
                                ...
                                let mat = Glass {
                                    color: Color::random(0.7, 1.0),
                                    refraction_index: rng.gen_range(0.75..1.5),
                                };
                                let obj = Sphere::new(center, 0.2, Box::new(mat));
                                world.push(Box::new(obj));
                            }
            ...
            "weekend-small" => {
                ...
                for x in -6..6 {
                    for z in -6..6 {
                        let randmat = rng.gen_range(0.0..1.0);
                        let center = Point3(
                            x as f64 * 2.0 + 1.8 * rng.gen_range(0.0..1.0),
                            0.2,
                            z as f64 * 2.0 + 1.8 * rng.gen_range(0.0..1.0),
                        );
            ...
            "mattest" => {
                ...
                let metal_clear_mat = Metal {
                    color: Color(0.0, 1.0, 0.0),
                    fuzziness: 0.0,
                };
                let metal_fuz_mat = Metal {
                    color: Color(0.0, 0.0, 1.0),
                    fuzziness: 0.5,
                };
                let glass_high_ri_mat = Glass {
                    color: Color(0.9, 0.9, 0.0),
                    refraction_index: 1.5,
                };
                let glass_low_ri_mat = Glass {
                    color: Color(0.9, 0.0, 0.9),
                    refraction_index: 0.75,
                };

                let matte = Sphere::new(Point3(-2.0, 0.0, -2.0), 0.5, Box::new(matte_mat));
                let metal_clear =
                    Sphere::new(Point3(0.0, 0.0, -2.0), 0.5, Box::new(metal_clear_mat));
                let metal_fuz = Sphere::new(Point3(2.0, 0.0, -2.0), 0.5, Box::new(metal_fuz_mat));
                let glass_high_ri =
                    Sphere::new(Point3(-1.2, 0.0, -2.5), 0.5, Box::new(glass_high_ri_mat));
                let glass_low_ri =
                    Sphere::new(Point3(1.2, 0.0, -2.5), 0.5, Box::new(glass_low_ri_mat));
    ...
    pub fn render(&mut self, file_path: &String) {
        ...
        self.camera
            .render(&mut self.world, file.expect("Failed to load file"));
    }
}
