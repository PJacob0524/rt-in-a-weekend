...
impl Material for Metal {
    fn scatter(&self, rin: &Ray, rec: &HitRecord) -> Option<Ray> {
        ...
        Some(Ray{origin: rec.p, direction: reflection})
    }
...
impl Material for Glass {
    fn scatter(&self, rin: &Ray, rec: &HitRecord) -> Option<Ray> {
        ...
        if ri * (1.0 - (cos_theta * cos_theta)).sqrt() > 1.0 || Self::schlick_approx(cos_theta, ri) {
            return Some(Ray{origin: rec.p, direction: unit_rin.reflect(&rec.normal)})
        }
        Some(Ray{origin: rec.p, direction: unit_rin.refract(&rec.normal, ri)})
    }
